...
export default class Utils {
  static isAppleMobile(): boolean {
    if ((["iPhone", "iPad", "iPod"].includes(navigator.platform) || (navigator.userAgent.match(/Mac/) && navigator.maxTouchPoints && navigator.maxTouchPoints > 2)) && "serviceWorker" in navigator) return true;
    return false;
  }
  ...
  static isAndroidFallback(): boolean {
    if (this.isAndroid() && !("BeforeInstallPromptEvent" in window)) return true;
    return false;
  }
  ...
  static isStandalone() {
    if (window.matchMedia("(display-mode: standalone)").matches || ("standalone" in navigator && (navigator as any).standalone === true)) return true;
    return false;
  }
  ...
  static setStorageFlag(name: string, value: boolean, persistent: boolean | null | undefined = false) {
    try {
      if (persistent) localStorage.setItem(name, value.toString());
      else if (persistent === false || persistent === undefined) sessionStorage.setItem(name, value.toString());
    } catch (e) {}
  }
  ...
  static async fetchAndProcessManifest(manifestUrl: string, icon: string, name: string, description: string): Promise<{ _manifest: WebAppManifest; icon: string; name: string; description: string }> {
    ...
    try {
      ...
      if (!_response.ok || !_json || !Object.keys(_json)) throw new Error("Manifest not found");
