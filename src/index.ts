...
export class PWAInstallElement extends LitElement {
  ...
  @property({ attribute: "no-set-storage", type: Boolean }) noSetStorage = false;
  ...
  public install = () => {
    if (this.isAppleMobilePlatform || this.isAppleDesktopPlatform) {
      ...
      this.requestUpdate();
    } else this._install.handleEvent();
  };
  /** @internal */
  private _hideDialog = {
    handleEvent: () => {
      this.isDialogHidden = true;
      Utils.setStorageFlag("pwa-hide-install", true, this.noSetStorage ? null : this.useLocalStorage);
      this.requestUpdate();
    },
    passive: true,
  };
  ...
  public showDialog = (forced = false) => {
    ...
    Utils.setStorageFlag("pwa-hide-install", false, this.noSetStorage ? null : this.useLocalStorage);
    this.requestUpdate();
  };
  ...
  private _toggleHowTo = {
    handleEvent: () => {
      ...
      if (this._howToRequested && this._galleryRequested) this._galleryRequested = false;
      this.requestUpdate();
  ...
  private _toggleGallery = {
    handleEvent: () => {
      ...
      if (this._howToRequested && this._galleryRequested) this._howToRequested = false;
  ...
  private _checkInstallAvailable() {
    ...
    if (!this.disableChrome && window.BeforeInstallPromptEvent) {
      ...
      if (this.externalPromptEvent != null) setTimeout(() => _promptHandler(this.externalPromptEvent!), 300);
      else window.addEventListener("beforeinstallprompt", _promptHandler);
    }
