  ...
  return html` <div id="pwa-install-element" dir="rtl">
    <div part="body" class="install-dialog chrome ${classMap(installDialogClasses())}" name="pwa mf ${installAvailable}">
      <div part="dialog-body" class="dialog-body">
        ...
        <div class="about" part="about">
          <div class="name" part="name">
            <label>${name}</label>
            ${!disableClose
              ? html`<button class="material-button secondary close" @click="${hideDialog}">
          ...
          <div class="hostname" part="hostname">${location.hostname}</div>
        </div>
        ${description ? html`<div part="description" class="description app-description">${description}</div>` : ""}
        ${!disableDescription
          ? html`<hr />
              <div class="description install-description">${installDescription ? installDescription : `${msg("This site has app functionality.")} ${msg("Install it on your device for extensive experience and easy access.")}`}</div>`
          : ""}
    ...
    ${!manualMobile
      ? html`<div class="install-dialog chrome mobile ${classMap(installDialogClasses())}">
          <pwa-bottom-sheet .props=${{ name, icon, description }} .disableClose=${disableClose} .install=${install} .hideDialog=${hideDialog} .toggleHowTo=${toggleHowTo} .howToRequested=${howToRequested} .fallback=${isAndroidFallback}> ${screenshotsAvailable && installAvailable ? html`<pwa-gallery .screenshots=${manifest.screenshots as ManifestScreenshot[]} .rtl="${isRTL}"></pwa-gallery>` : ""} </pwa-bottom-sheet>
        </div>`
      : ""}
  </div>`;
};
